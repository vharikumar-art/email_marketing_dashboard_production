...
def get_all_inr_rates(allow_live_fetch: bool = True) -> dict:
    ...
    Fetches in background to avoid blocking.
    """
    ...
    def fetch_rates_in_background():
        try:
            print("[Currency] Fetching current exchange rates from API in background...")
            ...
            print(f"[Currency] Exchange rates fetched and cached in background: {result}")
            
        except Exception as e:
            print(f"[Currency] Error fetching rates in background: {e}. Falling back to default rates temporarily.")
            cache_manager.set("exchange_rates:inr", DEFAULT_RATES, ttl=60)

    # Start background thread to fetch real rates
    threading.Thread(target=fetch_rates_in_background).start()
    
    # Return default rates immediately so the user isn't blocked
    return DEFAULT_RATES
